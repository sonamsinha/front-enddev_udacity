...
// When page is scrolled, requestAnimationFrame ge called
...
// Generates pizzas according to screen size
function makeLgPizza() {   
  var cols = 8;
  var s = 256;
  var windowHeight = (window.innerHeight || document.documentElement.clientHeight || 1080);
  var largePizzaGen = Math.ceil(windowHeight/s) * cols;
  for (var i = 0; i < largePizzaGen; i++) {
...
document.addEventListener('DOMContentLoaded', makeLgPizza);

// Re-generates number of pizzas according to screen size
window.addEventListener('resize', pizzaResize);